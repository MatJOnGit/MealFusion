...
final class UriUtils
{
    ...
    private function _checkUri()
    {
        try {
            ...
            if (!preg_match($this->_regexes['localhostUri'], $this->_uri)) {
                throw new EndpointException(400, 'Bad request');
            }

            /* en production, remplacer "/MealFusion/v1/" par "" */
            /* en local, remplacer "/v1/" par "" */
            $this->_strippedUri = str_replace('/v1/', '', $this->_uri);
        } catch (EndpointException $e) {
            $responseHandler = new ResponseHandler($e->getCode(), $e->getMessage());
            exit();
        } catch (Exception $e) {
